export const OPERATEURS = [{
  ...
  couleur: '#FFCC00',
  prefixes: ['67', '650', '651', '652', '653', '654', '680', '681', '682', '683', '684']
}, {
  id: 'ORANGE_CMR',
  label: 'Orange Money',
  couleur: '#FF7900',
  prefixes: ['69', '655', '656', '657', '658', '659', '685', '686', '687', '688', '689']
}];
...
export const detecterOperateur = phone => {
  const chiffres = normaliserChiffres(phone);
  if (chiffres.length < 2) return null;
  return OPERATEURS.find(op => op.prefixes.some(p => chiffres.startsWith(p))) || null;
};
